...
module.exports = class mysqlBase {
    constructor(connName, tableName) {
        let webconfig = global.webconfig;
        if (!conns[connName]) {
            conns[connName] = mysql.createPool(webconfig.mysql);
        }
        this.conn = conns[connName];
    ...
    getTableNames(tableName) {
        let sql="select table_name,table_comment,table_rows from information_schema.tables where table_schema='manage' and table_type='BASE TABLE'";
        if(tableName){
    ...
    findColumnName(tableName) {
        let sql='select column_name,column_comment,column_type from information_schema.columns where TABLE_SCHEMA=? and table_name=?';
